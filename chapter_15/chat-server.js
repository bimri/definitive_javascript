...
// The HTML file for the char client. Used below.
const clientHTML = fs.readFileSync("chatClient.html");

// An array of ServerResponse object that we're going to send event to
let clients = [];
...
server.on('request', (request, response) => {
    ...
    let pathname = urlString(request.url).pathname;
...
function acceptNewClient(request, response) {
    ...
    // Set headers and send an intial chat event to just this one client
...
async function broadcastNewMessage(request, response) {
    ...
    // and give it a double newline suffic that marks the end of the event.
