...
function initdb(db, callback) {
    ...
    // an option object that includes the "key path" specifying the 
    ...
    fetch("zipcodes.json")                      // Make an HTTP GET request
        ...
        .then(zipcodes => {                     // Get 40K zip code records
            ...
            // Get our object from the transaction
            let store = transaction.objectStore("zipcode");

            // The best part about the IndexDB API is that object stores
            // are *really* simple. Here's how we add (or update) our records:
            for(let record of zipcodes) { store.put(record); }

            // When the transacation completes successfully, the database
...
function lookupCity(zip, callback) {
    withDB(db => {  
        ...
        let transacation = db.transacation(["zipcodes"]);

        // Get the object store from the transcation
        let zipcodes = transacation.objectStore("zipcodes");
        ...
        request.onsuccess = () => {                 // Or call this function on success
            let record = request.result;            // This is the quesy result
